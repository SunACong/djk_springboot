...
/**
* @author JIAJIA
* @description 针对表【two_annea_fur】的数据库操作Service实现
* @createDate 2023-03-23 17:16:01
*/
@Service
public class TwoAnneaFurServiceImpl extends ServiceImpl<TwoAnneaFurMapper, TwoAnneaFur>
    implements TwoAnneaFurService{
    @Autowired
    private TwoAnneaFurMapper twoAnneaFurMapper;

    @Override
    public List<WarnTable> selectSpecial(LocalDateTime before, LocalDateTime after, String rollingName) {
        List<TwoAnneaFur> twoAnneaFurs = twoAnneaFurMapper.selectSpecial(before,after);
        ArrayList<WarnTable> warnTables = new ArrayList<>();
        twoAnneaFurs.forEach(i ->{
