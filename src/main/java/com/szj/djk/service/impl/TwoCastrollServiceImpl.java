 ...
 */
@Service
public class TwoCastrollServiceImpl extends ServiceImpl<TwoCastrollMapper, TwoCastroll> implements TwoCastrollService {
    @Autowired
    private TwoCastrollMapper twoCastrollMapper;
    @Override
    public List<TwoCastroll> selectRollingNewData() {
        return twoCastrollMapper.selectRollingNewData();
    }

    @Override
    public List<WarnTable> selectSpecial(LocalDateTime before, LocalDateTime after,String rollingName) {
        List<TwoCastroll> twoCastrolls = twoCastrollMapper.selectSpecial(before, after);
        System.out.println("这是拿到的special数据"+twoCastrolls);
        ArrayList<WarnTable> twoWarnTables = new ArrayList<>();
        twoCastrolls.forEach(item ->{
...
//            System.out.println("这是拿到的special数据"+item);
            switch (rollingName){
                case "上辊电机电流":
                    ...
                    twoWarnTables.add(warnTable);
                    System.out.println("这是拿到的列表"+twoWarnTables);
                    break;
                case "下辊电机电流":
                    warnTable.setRollingName(rollingName);
        ...
        System.out.println("这是警告数据"+twoWarnTables);
        return twoWarnTables;
    }
}
